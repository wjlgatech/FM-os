...
````yaml /openapi.json post /api/router/v2/query/start
openapi: 3.0.3
info:
  title: Nomadic API
  version: 1.0.0
...
paths:
  /api/router/v2/query/start:
    post:
      tags:
        - Analysis
      summary: Start analysis
      description: >-
        Start a prompt-based analysis over one or more videos and return an
        event stream id for progress updates.
      ...
      responses:
        '200':
          ...
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/AnalyzeStartResponse'
        '400':
