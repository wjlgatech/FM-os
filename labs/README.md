...
| [`rewardforge/`](rewardforge/) | **Eval failures → preference data → a real gated fine-tune**: chosen/rejected pairs labeled by FM-os's own certified gates (syndata-bare alignment, vlm-failure-probe), a from-scratch DPO loss (torch+peft, unit-tested math), LoRA on SmolLM2-135M, and a held-out grounding gate (zero word overlap with training — leakage is a tested property) that SHIPS or ROLLS BACK the tune. The "evals → training data → reward signals" bridge the OpenAI Personal AGI JDs name. **M3 (5 seeds, pre-registered median+separation rule): CONFIRMED — treatment +0.098±0.028 (median +0.110) vs control −0.046±0.055, complete arm separation; the stricter M2 every-seed rule still reads NO-EFFECT on one seed and both are reported ([out/M2-RESULTS.md](rewardforge/out/M2-RESULTS.md)).** | `make check` (offline, 8 tests) · `make e2e` (real fine-tune) · `python -m rewardforge.sweep` (M2) |
